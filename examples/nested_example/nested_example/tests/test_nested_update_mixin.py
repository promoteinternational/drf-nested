...
class NestedUpdateMixinTest(TestCase):
    def test_update_direct_nested_success(self):
        ...
        updated_employee = EmployeeSerializer(instance=employee.instance, data=data)
    ...
    def test_update_reverse_nested_success(self):
        user = UserGroupSerializer(
            data={"username": "Some name", "groups": [{"name": "First name"}]}
        )
        ...
        updated_user = UserGroupSerializer(instance=user.instance, data=data)
    ...
    def test_update_reverse_nested_error_captured(self):
        user = UserGroupSerializer(
            data={"username": "Some name", "groups": [{"name": "First name"}]}
        )
        user.is_valid(raise_exception=True)
    ...
    def test_update_many_to_many_nested_success(self):
        company = CompanySerializer(
            data={
                "name": "Company name",
                "managers": [{"user": {"username": "Some name"}, "level": "high"}],
            }
        )
        ...
        updated_company = CompanySerializer(instance=company.instance, data=data)
