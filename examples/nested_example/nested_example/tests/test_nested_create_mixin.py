...
class NestedCreateMixinTest(TestCase):
    def test_create_direct_nested_success(self):
        employee = EmployeeSerializer(
            data={"user": {"username": "Some name"}, "status": "Some status"}
        )
    ...
    def test_create_direct_nested_fail(self):
        employee = EmployeeSerializer(
            data={
                "user": {"username": "Some name", "is_active": False},
                "status": "Some status",
            }
        )
        employee.is_valid(raise_exception=True)
    ...
    def test_create_reverse_nested_success(self):
        user = UserGroupSerializer(
            data={
                "username": "Some name",
                "groups": [{"name": "First name"}, {"name": "Some name"}],
            }
        )
    ...
    def test_create_reverse_nested_error_capture(self):
        user = UserGroupErrorRaisingSerializer(
            data={
                "username": "Some name",
                "groups": [{"name": "First name"}, {"name": "Some name"}],
            }
    ...
    def test_create_reverse_nested_fail(self):
        user = UserGroupSerializer(
            data={
                ...
                "groups": [{"name": "Other group name"}, {"name": "Secondname"}],
            }
        )
        with self.assertRaises(ValidationError) as verror:
            user.is_valid(raise_exception=True)
            ...
            [
                {
                    "name": [
                        ErrorDetail(
                            string="Name shouldn't be greater than 10 symbols",
                            code="invalid",
                        )
                    ]
                },
    ...
    def test_create_many_to_many_nested_success(self):
        company = CompanySerializer(
            data={
                "name": "Company name",
                "managers": [{"user": {"username": "Some user name"}, "level": "high"}],
            }
        )
    ...
    def test_create_many_to_many_nested_fail(self):
        company = CompanySerializer(
            data={
                ...
                "managers": [
                    {
                        "user": {"username": "Some user name with username greater 20"},
                        "level": "high",
                    }
                ],
            }
        ...
        self.assertEqual(
            error.detail["managers"][0]["user"],
            {
                "username": [
                    ErrorDetail(
                        string="Username shouldn't be greater than 20 symbols",
                        code="invalid",
                    )
                ]
            },
    ...
    def test_create_generic_relation_nested_fail(self):
        ...
        self.assertEqual(
            error.detail["comments"][0],
            {
                "text": [
                    ErrorDetail(
                        string="Text shouldn't be greater than 20 symbols",
                        code="invalid",
                    )
                ]
            },
